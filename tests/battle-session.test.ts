...
const picks = ["AAPLc", "NVDAc", "TSLAc"].map((ticker, index) => ({ ticker, virtualAmount: index === 2 ? 33_334 : 33_333 }));
...
test("parses a valid persisted battle and rejects malformed state", () => {
  const session = createBattleSession({ playerDraftId: "draft-1", playerPicks: picks, rivalPicks: picks, openingPrices: prices });
  assert.equal(parseBattleSession(JSON.stringify(session))?.id, session.id);
