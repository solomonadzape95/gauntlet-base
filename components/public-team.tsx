...
type Entry = { id: string; name: string; tone: AvatarTone; lineup: { ticker: string; virtualAmount: number }[]; points: number | null; returnPercent: number | null; transferPenaltyPoints: number; joinedAt: string; week: { label: string; status: string } };
...
export function PublicTeam({ entryId }: { entryId: string }) {
  ...
  if (!entry) return <div className="shell page-shell"><p className="eyebrow hazard">LOADING TEAM…</p></div>;
  return <div className="shell page-shell public-team-page"><Link className="dashboard-text-link" href="/leaderboard"><ArrowLeft size={14} /> BACK TO RANKS</Link><section className="team-identity-hero public"><div className="team-identity-copy"><p className="eyebrow hazard">{entry.week.label} · {entry.week.status}</p><div className="team-name-lockup"><DitherAvatar seed={`${entry.name}:${entry.id}`} tone={entry.tone} size={82} /><div><span>TEAM</span><h1>{entry.name}</h1></div></div></div><div className="team-crest" aria-hidden><DitherAvatar seed={`${entry.name}:crest`} tone={entry.tone} size={230} /></div><div className="team-metrics"><span><small>POINTS</small><strong>{entry.points?.toLocaleString() ?? "LIVE"}</strong></span><span><small>RETURN</small><strong>{entry.returnPercent == null ? "—" : `${entry.returnPercent >= 0 ? "+" : ""}${entry.returnPercent.toFixed(2)}%`}</strong></span><span><small>BANK</small><strong>{squadBank(entry.lineup)} CR</strong></span></div></section><section className="squad-board"><div className="squad-board-heading"><div><p className="eyebrow">GAME WEEK SNAPSHOT</p><h2>THE LOCKED LINEUP</h2></div>{entry.transferPenaltyPoints ? <span>−{entry.transferPenaltyPoints} TRANSFER PTS</span> : <span><Trophy size={14} /> CLEAN SHEET</span>}</div><div className="squad-strips">{entry.lineup.map((pick, index) => { const stock = getStock(pick.ticker); return stock && <article key={pick.ticker} style={{ "--stock-tone": stock.tone } as React.CSSProperties}><span className="squad-number">0{index + 1}</span><span className="squad-logo"><StockLogo ticker={pick.ticker} /></span><span className="squad-company"><small>{stock.sector}</small><strong>{stock.company}</strong></span><span className="squad-price"><small>DRAFT COST</small><strong>{pick.virtualAmount} CR</strong></span></article>; })}</div></section></div>;
}
