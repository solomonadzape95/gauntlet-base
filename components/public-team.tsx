...
export function PublicTeam({ entryId }: { entryId: string }) {
  ...
  return <div className="shell page-shell public-team-page"><Link className="dashboard-text-link" href="/leaderboard"><ArrowLeft size={14} /> BACK TO RANKS</Link><section className="team-identity-hero public"><div className="team-identity-copy"><p className="eyebrow hazard">{entry.week.label} · {entry.week.status}</p><div className="team-name-lockup"><DitherAvatar seed={entry.name} tone={entry.tone} size={82} /><div><span>TEAM</span><h1>{entry.name}</h1></div></div></div><div className="team-metrics"><span><small>POINTS</small><strong>{entry.points?.toLocaleString() ?? "0"}</strong></span><span><small>RETURN</small><strong>{entry.returnPercent == null ? "0.00%" : `${entry.returnPercent >= 0 ? "+" : ""}${entry.returnPercent.toFixed(2)}%`}</strong></span><span><small>BANK</small><strong>{formatVirtualMoney(squadBank(entry.lineup))}</strong></span></div><div className="team-performance"><small>RECORDED PERFORMANCE</small><PerformanceSparkline history={entry.history} label={`${entry.name} score`} /></div></section><section className="squad-board"><div className="squad-board-heading"><div><p className="eyebrow">GAME WEEK SNAPSHOT</p><h2>THE LOCKED LINEUP</h2></div>{entry.transferPenaltyPoints ? <span>−{entry.transferPenaltyPoints} TRANSFER PTS</span> : <span><Trophy size={14} /> CLEAN SHEET</span>}</div><div className="squad-strips">{entry.lineup.map((pick, index) => { const stock = getStock(pick.ticker); return stock && <button type="button" className="squad-stock-row" key={pick.ticker} onClick={() => setProofTicker(pick.ticker)} aria-label={`Open ${stock.company} onchain details`} style={{ "--stock-tone": stock.tone } as React.CSSProperties}><span className="squad-number">0{index + 1}</span><span className="squad-logo"><StockLogo ticker={pick.ticker} /></span><span className="squad-company"><small>{stock.sector}</small><strong>{stock.company}</strong></span><span className="squad-price"><small>DRAFT COST</small><strong>{formatVirtualMoney(pick.virtualAmount)}</strong></span><ArrowRight className="squad-open" size={16} /></button>; })}</div></section><StockProofPanel ticker={proofTicker} savedCost={entry.lineup.find((pick) => pick.ticker === proofTicker)?.virtualAmount} onClose={() => setProofTicker(null)} /></div>;
}
