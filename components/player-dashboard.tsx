...
export function PlayerDashboard() {
  ...
  return <div className="dashboard-shell shell page-shell player-hub">
    ...
    <section className="dashboard-panel player-team-card">
      ...
      {team && <div className="dashboard-holdings">{team.picks.map((pick) => { const stock = getStock(pick.ticker); if (!stock) return null; const currentCost = market.find((item) => item.ticker === pick.ticker)?.draftCost; return <div key={pick.ticker}><span className="holding-logo" style={{ color: stock.logoColor }}><StockLogo ticker={pick.ticker} /></span><span><strong>{stock.company}</strong><small>{pick.ticker}</small></span><strong>{currentCost ? `${currentCost} CR` : "HELD"}</strong></div>; })}</div>}
