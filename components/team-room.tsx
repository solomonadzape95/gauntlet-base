  ...
  return (
    <div className="team-room">
      <section className="team-identity-hero">
        ...
        <div className="team-metrics"><span><small>GAME WEEK PTS</small><strong>{viewerPoints?.toLocaleString() ?? "0"}</strong></span><span><small>STOCKS</small><strong>{team.picks.length}</strong></span><span><small>BANK</small><strong>{formatVirtualMoney(state?.bank ?? squadBank(team.picks))}</strong></span></div>
      </section>
      ...
      {tab === "squad" && <section className="squad-board">
        <div className="squad-board-heading"><div><p className="eyebrow">ACTIVE TEAM</p><h2>{team.picks.length} STOCKS ON THE FLOOR</h2></div><span>{formatVirtualMoney(squadBank(team.picks))} BANKED</span></div>
        <div className="squad-strips">{team.picks.map((pick, index) => {
          const stock = getStock(pick.ticker);
          return stock && <button type="button" className="squad-stock-row" key={pick.ticker} onClick={() => setProofTicker(pick.ticker)} aria-label={`Open ${stock.company} onchain details`} style={{ "--stock-tone": stock.tone } as React.CSSProperties}><span className="squad-number">0{index + 1}</span><span className="squad-logo"><StockLogo ticker={pick.ticker} /></span><span className="squad-company"><small>{stock.sector}</small><strong>{stock.company}</strong></span><span className="squad-price"><small>COST</small><strong>{formatVirtualMoney(pick.virtualAmount)}</strong></span><ArrowRight className="squad-open" size={16} /></button>;
        })}</div>
      </section>}

      {tab === "transfers" && <section className="transfer-desk">
        <header><div><p className="eyebrow hazard">TRANSFERS</p><h2>SWAP OR ADD STOCKS</h2></div><div className={`transfer-budget ${bank < 0 ? "over" : ""}`}><small>BANK</small><strong>{formatVirtualMoney(bank)}</strong><span>{bank < 0 ? "OVER BUDGET" : projectedPenalty ? `−${projectedPenalty} PTS` : "FREE MOVE AVAILABLE"}</span></div></header>
      ...
      {tab === "market" && <section className="market-ledger"><div className="squad-board-heading"><div><p className="eyebrow">ONCHAIN MARKET</p><h2>EVERY AVAILABLE STOCK</h2></div><span>PRICES REFRESH SERVER-SIDE</span></div><div>{STOCKS.map((stock) => {
        ...
        return <button type="button" className="market-stock-row" key={stock.ticker} onClick={() => setProofTicker(stock.ticker)} aria-label={`Open ${stock.company} onchain details`} style={{ "--stock-tone": stock.logoColor } as React.CSSProperties}><span className="transfer-logo"><StockLogo ticker={stock.ticker} /></span><span><strong>{stock.company}</strong><small>{stock.ticker}</small></span><strong>{quote ? formatVirtualMoney(quote.draftCost) : "HELD"}</strong><small>{quote ? `$${quote.price.toFixed(2)} REFERENCE` : "WAITING FOR FEED"}</small><ArrowRight size={15} /></button>;
      })}</div></section>}
