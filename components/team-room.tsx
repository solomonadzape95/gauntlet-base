...
export function TeamRoom({ ownDraftId, returnTo, preview = false }: { ownDraftId?: string; returnTo?: string; preview?: boolean }) {
  ...
  const liveForm = useMemo(() => team ? team.picks.reduce((sum, pick) => sum + (MARKET_QUOTES.find((quote) => quote.ticker === pick.ticker)?.change ?? 0) * (pick.virtualAmount / 1_000), 0) : 0, [team]);

  if (active.loading && !ownDraftId && !preview) return <div className="team-room-loading">LOADING TEAM…</div>;
  ...
  function toggle(ticker: string) {
    ...
    setSelected((current) => {
      ...
      const cost = market.find((item) => item.ticker === ticker)?.draftCost ?? 1_001;
      return current.length < 5 && selectionCost + cost <= 1_000 ? [...current, ticker] : current;
    });
  }
    ...
    <div className="team-room">
      <section className="team-identity-hero">
        <div className="team-identity-copy">
          ...
          <div className="team-name-lockup"><DitherAvatar seed={`${playerName}:${profile?.user_id ?? "guest"}`} tone={profile?.avatar_tone ?? "hazard"} size={82} /><div><span>MANAGER</span><h1>{playerName}</h1></div></div>
        ...
        <div className="team-crest" aria-hidden><DitherAvatar seed={`${playerName}:crest`} tone={profile?.avatar_tone ?? "hazard"} size={230} /></div>
        <div className="team-metrics"><span><small>GAME WEEK PTS</small><strong>{viewerPoints?.toLocaleString() ?? "—"}</strong></span><span><small>LIVE FORM</small><strong className={liveForm >= 0 ? "up" : "down"}>{liveForm >= 0 ? "+" : ""}{liveForm.toFixed(2)}%</strong></span><span><small>BANK</small><strong>{state?.bank ?? squadBank(team.picks)} CR</strong></span></div>
      </section>
      ...
      {tab === "squad" && <section className="squad-board">
        ...
        <div className="squad-strips">{team.picks.map((pick, index) => {
          ...
          const form = MARKET_QUOTES.find((quote) => quote.ticker === pick.ticker)?.change ?? 0;
          return stock && <article key={pick.ticker} style={{ "--stock-tone": stock.tone } as React.CSSProperties}><span className="squad-number">0{index + 1}</span><span className="squad-logo"><StockLogo ticker={pick.ticker} /></span><span className="squad-company"><small>{stock.sector}</small><strong>{stock.company}</strong></span><span className="squad-form"><small>FORM</small><strong className={form >= 0 ? "up" : "down"}>{form >= 0 ? "+" : ""}{form.toFixed(2)}%</strong></span><span className="squad-price"><small>COST</small><strong>{pick.virtualAmount} CR</strong></span></article>;
        })}</div>
      </section>}

      {tab === "transfers" && <section className="transfer-desk">
        <header><div><p className="eyebrow hazard">TRANSFER WINDOW</p><h2>RESHAPE THE TEAM</h2><p>One incoming stock is free each Game Week. Every additional incoming stock deducts 25 points from that week.</p></div><div className="transfer-budget"><small>BANK AFTER MOVES</small><strong>{bank} CR</strong><span>{projectedPenalty ? `−${projectedPenalty} PTS` : "FREE MOVE AVAILABLE"}</span></div></header>
        ...
        <div className="transfer-list">{STOCKS.map((stock) => {
          ...
          return <button key={stock.ticker} disabled={!state?.transferWindow.open || !quote || unaffordable || (!chosen && selected.length >= 5)} onClick={() => toggle(stock.ticker)} className={chosen ? "selected" : ""}><span className="transfer-logo"><StockLogo ticker={stock.ticker} /></span><span><strong>{stock.company}</strong><small>{stock.ticker} · {stock.sector}</small></span><span><small>DRAFT COST</small><strong>{quote ? `${quote.draftCost} CR` : "HELD"}</strong></span><span className="transfer-action">{chosen ? "REMOVE" : unaffordable ? "OVER BUDGET" : "ADD"}</span></button>;
        })}</div>
