    ...
    <section className="stock-proof-panel" role="dialog" aria-modal="true" aria-labelledby="stock-proof-title" style={{ "--stock-tone": stock.logoColor } as React.CSSProperties}>
      ...
      <>
        <div className="stock-proof-metrics">
          ...
          <article><small>DRAFT COST</small><strong>{savedCost || quote?.draftCost ? formatVirtualMoney(savedCost ?? quote!.draftCost) : "—"}</strong><span>{marketQuote ? "CURRENT MARKET" : savedCost ? "LOCKED GAME WEEK" : "CURRENT MARKET"}</span></article>
