...
export function LeaguesPanel() {
  ...
  return <div className="shell page-shell leagues-page">
    ...
    <div className="league-grid">{leagues.map((league) => {
      ...
      const expanded = expandedLeague === league.id;
      const visibleMembers = members.slice(0, 4);
      return <section className={`league-card dashboard-panel ${expandedLeague === league.id ? "expanded" : ""}`} key={league.id}>
        <header><span className="league-shield"><Shield size={24} /></span><div><p className="eyebrow">YOUR LEAGUE · {league.members.length} PLAYERS</p><h2>{league.name}</h2></div><button onClick={() => void copy(league.join_code)}>{copied === league.join_code ? <Check size={14} /> : <Copy size={14} />}{league.join_code}</button></header>
        ...
        {!expanded && <div className="league-members">{visibleMembers.map((member, index) => { const shownPoints = quick ? member.quickPoints : member.points; const shownRank = quick ? member.quickRank ?? index + 1 : index + 1; return <div key={`${member.name}:${member.joinedAt}`}><strong>{String(shownRank).padStart(2, "0")}</strong><DitherAvatar seed={member.name} tone={member.tone} size={34} /><span>{member.teamEntryId ? <Link href={`/team/${member.teamEntryId}`}><b>{member.name}</b></Link> : <b>{member.name}</b>}<small>{quick ? quick.status === "active" ? "QUICK LEAGUE LIVE" : "QUICK LEAGUE FINAL" : shownPoints != null && index === 0 ? "LEAGUE LEADER" : "MEMBER"}</small></span><em>{quick ? signedPoints(shownPoints) : shownPoints == null ? "— PTS" : `${shownPoints.toLocaleString()} PTS`}</em></div>; })}</div>}
        <button className="league-view-toggle" onClick={() => setExpandedLeague((current) => current === league.id ? null : league.id)}>{expanded ? <ChevronUp size={15} /> : <ChevronDown size={15} />} {expanded ? "CLOSE LEAGUE" : members.length > 4 ? `VIEW ALL ${members.length} PLAYERS` : "VIEW LEAGUE"}</button>
        {expanded && <div className="league-inside">
