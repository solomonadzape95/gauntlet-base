...
import { Check, Copy, Plus, Shield, TimerReset, UserPlus, Zap } from "lucide-react";
...
type Member = { name: string; tone: AvatarTone; joinedAt: string; teamEntryId: string | null; points: number | null; quickPoints: number | null; quickRank: number | null };
type League = { id: string; name: string; join_code: string; isOwner: boolean; quickRound: { status: "active" | "complete"; startsAt: string; endsAt: string } | null; members: Member[] };
...
export function LeaguesPanel() {
  ...
  return <div className="shell page-shell leagues-page">
    ...
    <div className="league-grid">{leagues.map((league) => {
      ...
      return <section className="league-card dashboard-panel" key={league.id}>
        ...
        <div className={`league-sprint ${quick?.status ?? "ready"}`}><span><Zap size={16} /><b>QUICK LEAGUE</b><small>{quick ? quick.status === "active" ? `LIVE · ENDS ${new Date(quick.endsAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}` : "FINAL · FIVE-MINUTE TABLE" : "REAL FEEDS · FIVE MINUTES"}</small></span>{league.isOwner && quick?.status !== "active" && <button disabled={working || !quickLeagueReady} onClick={() => void startQuickLeague(league.id)}><TimerReset size={15} /> {quick ? "RUN AGAIN" : "START 5 MIN"}</button>}</div>
        <div className="league-members">{members.map((member, index) => { const shownPoints = quick ? member.quickPoints : member.points; const shownRank = quick ? member.quickRank ?? index + 1 : index + 1; return <div key={`${member.name}:${member.joinedAt}`}><strong>{String(shownRank).padStart(2, "0")}</strong><DitherAvatar seed={member.name} tone={member.tone} size={34} /><span>{member.teamEntryId ? <Link href={`/team/${member.teamEntryId}`}><b>{member.name}</b></Link> : <b>{member.name}</b>}<small>{quick ? quick.status === "active" ? "QUICK LEAGUE LIVE" : "QUICK LEAGUE FINAL" : shownPoints != null && index === 0 ? "LEAGUE LEADER" : "MEMBER"}</small></span><em>{shownPoints == null ? "— PTS" : `${shownPoints.toLocaleString()} PTS`}</em></div>; })}</div>
      </section>;
    })}</div>
