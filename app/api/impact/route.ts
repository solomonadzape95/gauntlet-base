...
export async function GET() {
  ...
  const [purchases, drafts, battles, wallets, ledger] = await Promise.all([
    supabase.from("purchase_attempts").select("id", { count: "exact", head: true }).eq("status", "confirmed").eq("balance_verified", true).not("transaction_hash", "is", null).not("confirmed_at", "is", null),
    ...
    supabase.from("purchase_attempts").select("ticker,transaction_hash,confirmed_at").eq("status", "confirmed").eq("balance_verified", true).not("transaction_hash", "is", null).not("confirmed_at", "is", null).order("confirmed_at", { ascending: false }).limit(20),
  ]);
  ...
  const rows: ImpactLedgerRow[] = (ledger.data ?? []).flatMap((row) => (
    typeof row.transaction_hash === "string" && typeof row.confirmed_at === "string"
      ? [{ ticker: row.ticker, transactionHash: row.transaction_hash, confirmedAt: row.confirmed_at }]
      : []
  ));
